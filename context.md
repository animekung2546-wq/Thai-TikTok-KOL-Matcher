...
- Test suite: `23 passed` on the last verified run
...
- `brand_analyzer.py`: Fetches public page text and extracts a structured brand profile with heuristic fallback.
- `kol_data.py`: Loads the bundled KOL dataset and provides the optional Apify normalization boundary.
...
- `tests/test_brand_analyzer.py`: Brand extraction, fetch fallback, HTML cleanup, source warnings.
- `tests/test_kol_data.py`: CSV schema, sample metadata, Apify fallback warnings, URL normalization.
